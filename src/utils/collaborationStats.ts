...
export function calculateCollaborationStats(repos: Repository[]): CollaborationStats {
  ...
  const collaboratorDistribution = collaboratorRanges.map(range => ({
    range: range.label,
    count: repos.filter(repo => 
      (repo.Collaborator_Count || 0) >= range.min && (repo.Collaborator_Count || 0) <= range.max
    ).length
  }));
  ...
  const featureDistribution = [
    ...
    },
    {
      ...
      count: repos.filter(repo => (repo.Protected_Branch_Count || 0) > 0).length,
      total: repos.length
    },
    {
      feature: 'Milestones',
