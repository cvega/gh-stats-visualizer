...
export function calculateStats(repos: Repository[]): Stats {
  const basic = {
    ...
    totalPRs: repos.reduce((sum, repo) => sum + (repo.Pull_Request_Count || 0), 0),
    totalCommitComments: repos.reduce((sum, repo) => sum + (repo.Commit_Comment_Count || 0), 0),
    totalMilestones: repos.reduce((sum, repo) => sum + (repo.Milestone_Count || 0), 0),
    totalReleases: repos.reduce((sum: number, repo) => sum + (repo.Release_Count || 0), 0),
    totalCollaborators: repos.reduce((sum: number, repo) => sum + (repo.Collaborator_Count || 0), 0),
    totalProtectedBranches: repos.reduce((sum: number, repo) => sum + (repo.Protected_Branch_Count || 0), 0),
    totalProjects: repos.reduce((sum: number, repo) => sum + (repo.Project_Count || 0), 0),
    totalWikis: repos.reduce((sum: number, repo) => sum + (repo.Has_Wiki || 0), 0),
  ...
  const activityData = repos.map(repo => ({
    name: repo.Repo_Name || '',
  ...
  const updateData = repos.map(repo => ({
    name: repo.Repo_Name || '',
  ...
  const orgData = Array.from(orgMap.entries()).map(([name, value]) => ({ name, value }));
  ...
  const branchData = repos.map(repo => ({
    name: repo.Repo_Name || '',
  ...
  const metadataRatios = repos.map(repo => ({
    name: repo.Repo_Name || '',
    size: repo.Repo_Size_MB || 0,
    metadata: (repo.Issue_Count || 0) + (repo.Pull_Request_Count || 0) + (repo.Commit_Comment_Count || 0),
    ratio: ((repo.Issue_Count || 0) + (repo.Pull_Request_Count || 0) + (repo.Commit_Comment_Count || 0)) / (repo.Repo_Size_MB || 1)
  })).sort((a, b) => b.ratio - a.ratio);

  // Calculate branch complexity
  const branchComplexity = repos.map(repo => ({
    name: repo.Repo_Name || '',
    branches: repo.Branch_Count || 0,
    size: repo.Repo_Size_MB || 0,
    age: repo.Created || '',
    complexityBySize: (repo.Branch_Count || 0) / (repo.Repo_Size_MB || 1),
    complexityByAge: (repo.Branch_Count || 0) / (new Date().getTime() - new Date(repo.Created || '').getTime())
  })).sort((a, b) => b.complexityBySize - a.complexityBySize);

  // Calculate tag release frequency
  const tagReleaseFrequency = repos.map(repo => ({
    name: repo.Repo_Name || '',
    tags: repo.Tag_Count || 0,
    releases: repo.Release_Count || 0,
    age: repo.Created || '',
    tagsPerYear: (repo.Tag_Count || 0) / ((new Date().getTime() - new Date(repo.Created || '').getTime()) / (1000 * 60 * 60 * 24 * 365)),
    releasesPerYear: (repo.Release_Count || 0) / ((new Date().getTime() - new Date(repo.Created || '').getTime()) / (1000 * 60 * 60 * 24 * 365)),
    total: (repo.Tag_Count || 0) + (repo.Release_Count || 0)
  })).sort((a, b) => b.total - a.total);

  // Calculate repository age
  const repositoryAge = repos.map(repo => ({
    name: repo.Repo_Name || '',
    ageInDays: Math.floor((new Date().getTime() - new Date(repo.Created || '').getTime()) / (1000 * 60 * 60 * 24)),
    ageInYears: Math.floor((new Date().getTime() - new Date(repo.Created || '').getTime()) / (1000 * 60 * 60 * 24 * 365))
